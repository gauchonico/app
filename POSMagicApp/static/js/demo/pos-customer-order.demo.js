...
function showProductModal(productId, productName, stock) {
    ...
    // Show loading state in price group select
    const priceGroupSelect = document.getElementById('priceGroup');
    priceGroupSelect.innerHTML = '<option value="">Loading price groups...</option>';
    ...
    fetch(url)
        ...
        .then(data => {
            console.log('Received price groups:', data);
            
            // Reset select options
            priceGroupSelect.innerHTML = '<option value="">Select a Price Group</option>';
            ...
            if (data.price_groups && data.price_groups.length > 0) {
                data.price_groups.forEach(group => {
                    ...
                    option.textContent = `${group.name} - ${parseFloat(group.price).toFixed(2)}`;
                    if (group.is_active) {
                        option.selected = true;
                    }
                    priceGroupSelect.appendChild(option);
                });
                priceGroupSelect.onchange = updateProductPrice;
                // Update displayed price based on selection
                updateProductPrice();
            } else {
                priceGroupSelect.innerHTML = '<option value="">No price groups available</option>';
            }
        })
        .catch(error => {
            console.error('Error fetching price groups:', error);
            priceGroupSelect.innerHTML = '<option value="">Error loading price groups</option>';
        });
...
function addProductToCart() {
    ...
    const productItem = {
        ...
        quantity: quantity,
        price_group_id: selectedOption.value,
        price_group_name: selectedOption.textContent.split(' - ')[0],
        unit_price: price,
...
function addAccessoryToCart() {
    ...
    const accessoryItem = {
        ...
        quantity: quantity,
        unit_price: parseFloat(selectedAccessory.price),  // Store unit price separately
